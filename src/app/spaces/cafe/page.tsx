...
export default function CafeWorkZonesPage() {
  const spaces = getSpacesByType("cafe")

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
          <Coffee className="h-7 w-7 text-[#0F7377] dark:text-teal-400" />
          Cafe Work Zones
        </h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm mt-0.5">
          Quiet corners with power outlets and good WiFi.
        </p>
      </div>
      {spaces.length === 0 ? (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-12 text-center">
          <p className="text-slate-600 dark:text-slate-400">No cafe work zones yet.</p>
